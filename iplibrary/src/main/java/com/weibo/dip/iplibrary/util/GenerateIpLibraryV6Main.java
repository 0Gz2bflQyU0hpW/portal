 ...
 * @author yurun
 */
public class GenerateIpLibraryV6Main {
  private static final Logger LOGGER = LoggerFactory.getLogger(GenerateIpLibraryV6Main.class);
  ...
  public static class IpData {
    private String startIpInHex;
    private String endIpInInHex;
     ...
     *
     * @param startIpInHex start ip, represendted as an hex(include)
     * @param endIpInInHex end ip, represented as an hex(include)
    ...
    public IpData(
        ...
        String type,
        String desc) {
      this.startIpInHex = startIpInHex;
      this.endIpInInHex = endIpInInHex;
    ...
    public String getStartIpInHex() {
      return startIpInHex;
    }

    public String getEndIpInInHex() {
      return endIpInInHex;
    }
  ...
  private static final SortedMap<String, IpData> IPDATAS = new TreeMap<>();

  private static final String[] IPPOINTS;
  ...
  static {
    try {
      ...
      IPPOINTS = IPDATAS.keySet().toArray(new String[0]);
    } catch (Exception e) {
  ...
  private static void initializeCountries() throws Exception {
    ...
    InputStream xmlIn = GenerateIpLibraryV6Main.class.getResourceAsStream("/region.xml");
  ...
  private static void initializeIsps() throws Exception {
    ...
    InputStream xmlIn = GenerateIpLibraryV6Main.class.getResourceAsStream("/isp.xml");
  ...
  private static void initializeTypes() throws Exception {
    ...
    InputStream xmlIn = GenerateIpLibraryV6Main.class.getResourceAsStream("/type.xml");
  ...
  private static void initializeIpDatas() throws Exception {
    InputStream in = GenerateIpLibraryV6Main.class.getResourceAsStream("/ip6data.csv");
    ...
    try (BufferedReader reader = new BufferedReader(new InputStreamReader(in))) {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        try {
          String startIpInHex = words[0].substring(2);
          String endIpInHex = words[1].substring(2);
  ...
  public static SortedMap<String, IpData> getIpdatas() {
    return IPDATAS;
  }
  ...
  public static void create(String indexDir) throws Exception {
    ...
    try (BufferedWriter ipPointsWriter =
      ...
      Map<String, Country> countries = GenerateIpLibraryV6Main.getCountries();
      Map<String, Isp> isps = GenerateIpLibraryV6Main.getIsps();
      Map<String, Type> types = GenerateIpLibraryV6Main.getTypes();
      SortedMap<String, IpData> ipdatas = GenerateIpLibraryV6Main.getIpdatas();

      ipPointsWriter.write(String.valueOf(ipdatas.size()));
      ipPointsWriter.newLine();

      for (Entry<String, IpData> entry : ipdatas.entrySet()) {
        IpData ipData = entry.getValue();

        ipPointsWriter.write(String.valueOf(ipData.getEndIpInInHex()));
        ipPointsWriter.newLine();
        ...
        String startIpCol = ipData.getStartIpInHex();
        String endIpCol = ipData.getEndIpInInHex();
