...
public class ClasspathProperties {
  ...
  public ClasspathProperties(String fileName) throws Exception {
    URL resource = ClasspathProperties.class.getClassLoader().getResource(fileName);
    Preconditions.checkState(Objects.nonNull(resource), "classpath:%s does not exist", fileName);
      ...
      reader =
          new BufferedReader(
              new InputStreamReader(
                  new FileInputStream(new File(resource.toURI())), CharEncoding.UTF_8));
