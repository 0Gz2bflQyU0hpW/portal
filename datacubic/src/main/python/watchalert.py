...
def usage(return_code=2):
    help = """
Usage %s --sv "monitor" --rpcService "rpcService" --object "object" --subject "subject" --msgto "someone,otherone" [options]
...
def sendAlertToUsers(rpcService, subservice, subject, content, users, weiboto=True, mailto=True, msgto=False):
    if not (rpcService and subservice and subject and users):
        logging.warn(
            "illegal alert msg, rpcService: %s, object: %s, subject: %s, content: %s, users: %s")
        return
    params = dict(
        kid="2012090416",
...
def sendAlertToGroups(rpcService, subservice, subject, content, groups, weiboto=True, mailto=True, msgto=False):
    if not (rpcService and subservice and subject and groups):
        logging.warn(
            "illegal alert msg, rpcService: %s, object: %s, subject: %s, content: %s, groups: %s")
        return
    params = dict(
        kid="2012090416",
        passwd="VrcUe70eTvnGIm3wd4g6cLtMlvg7s7",
...
if __name__ == "__main__":
    ...
    parser.add_argument("-rpcService", "--rpcService", type=str,
                        action="store", required=True, help="rpcService")
    ...
    if args.type == "user":
        sendAlertToUsers(args.rpcService, args.subservice, args.subject,
                         args.content, args.receivers, args.weibo, args.mail, args.msg)
    elif args.type == "group":
        sendAlertToGroups(args.rpcService, args.subservice, args.subject,
                          args.content, args.receivers, args.weibo, args.mail, args.msg)
    else:
        parser.print_help()
