...
public class DipStreaming {
    ...
    public static void start(StreamingContext context) {
        ...
        try {
            String status = jedis.hget(SPARK_STREAMING, appName);
            if (!status.equals(STOPED)) {
                LOGGER.warn("streaming {} app already running", appName);
    ...
    public static void stoped(String appName) {
        Jedis jedis = new Jedis(REDIS_HOST, REDIS_PORT);
    ...
    public static void main(String[] args) {
        System.out.println(isStarted("VideoUploadStreamingV1"));
        System.out.println(isStoped("VideoUploadStreamingV1"));

        stop("VideoUploadStreamingV1");
        stoped("VideoUploadStreamingV1");
    }
