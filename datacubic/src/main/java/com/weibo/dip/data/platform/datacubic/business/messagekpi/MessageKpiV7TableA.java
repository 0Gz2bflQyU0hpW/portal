...
/**
 * tcp.
 * Created by liyang28 on 2018/6/14.
 */
public class MessageKpiV7TableA {

  private static final Logger LOGGER = LoggerFactory.getLogger(MessageKpiV7TableA.class);
  ...
  public static String getInputPath(String date) {
    return "/user/hdfs/rawlog/www_weibomessageboxnt71g_personalstatisticlog/" + date + "/*";
  }
      ...
      reader = new BufferedReader(
          new InputStreamReader(
              Ideo_Download_Base2slow.class.getClassLoader()
                  .getResourceAsStream(MessageKpiCommon.V7_TABLE_A),
              CharEncoding.UTF_8));
      String line;
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      session.udf().register(MessageKpiCommon.IP_TO_LOCATION, new IpToLocation(),
          DataTypes.createMapType(DataTypes.StringType, DataTypes.StringType));
      String[] fieldNames =
          {"name", "success_count", "total_count", "avg_time"};
      ...
      JavaRDD<Row> rowRdd = MessageKpiCommon
          .getNotJsonDataRddTcp(sc, getInputPath(date), fieldNames, notMatchCount);
      ...
      MessageKpiCommon
          .sendKafka(resultDs, date, outputCount, MessageKpiCommon.V7_TABLE_A_BUSINESS);
