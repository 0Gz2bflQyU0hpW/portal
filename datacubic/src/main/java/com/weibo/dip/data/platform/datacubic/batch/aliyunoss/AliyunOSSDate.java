...
public class AliyunOSSDate {
    private static Logger LOGGER = LoggerFactory.getLogger(AliyunOSSDate.class);
...
//                sparkSession.udf().register("getdayUTC",new getDayUTC(),DataTypes.StringType);
                sparkSession.udf().register("time_to_utc_with_interval", new TimeToUTCWithInterval(), DataTypes.StringType);
            ...
            List<Row> resultRow = results.javaRDD().collect();

            if (CollectionUtils.isEmpty(resultRow)){
                LOGGER.info("result size = 0,return.");
            }

            String path = "/user/tmp/aliyunOssFindDate";
